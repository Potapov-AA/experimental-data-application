...
class Analysis:
    ...
    def ImageHistogram(self, imageData):
        plt.figure(figsize=(15, 5))
        plt.title("Гистограмма")
        plt.hist(imageData.ravel(), bins=256, rwidth=0.8, range=(0, 255))
        plt.show()
    
    # Классическая гистограмма с предаврительным приведением к серому 
    def ImageHistogramWithToGray(self, imageData):        
        maxPixel = imageData.max();
        minPixel = imageData.min();
        
        for i in range(len(imageData)):
            for j in range(len(imageData[i])):
                imageData[i][j] = ((imageData[i][j] - minPixel) / (maxPixel - minPixel)) * 255
    ...
    # RGB гистограмма
    def ImageRGBHistogram(self, imageData):        
        rList = []
        gList = []
        bList = []
        ...
        plt.hist(rList, bins=256, rwidth=0.8, range=(0, 255), color='r')
        plt.hist(gList, bins=256, rwidth=0.8, range=(0, 255), color='g')
        plt.hist(bList, bins=256, rwidth=0.8, range=(0, 255), color='b')
        plt.show()
